...
def test_parse_rclone_uri():
    remote, host, user, pw, port, path = parse_rclone_uri(
        "rclone://user:pass@remote:22/path"
    )
...
def test_transfer_with_rclone(
    mock_run, mock_spot_check_verify, mock_directory_size_verify
):
